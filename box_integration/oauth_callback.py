...
import config
from box_integration.box_oauth import oauth_authenticate
...
def callback_handle_request(csrf_token: str):
    """
    Handles the call back request from Box OAuth2.0
    ...
    web_server = HTTPServer(
        (config.CALLBACK_HOSTNAME, int(config.CALLBACK_PORT)), CallbackServer
    )
